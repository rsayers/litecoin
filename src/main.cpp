...
#if defined(NDEBUG)
# error "Newyen cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xabea72a3051ca1116c6121adbb90116c77391cb009975746724da439deef348d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Newyen: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Newyen Signed Message:\n";
...
{
    // Newyen: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 4000000000000 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840000); // Newyen: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Newyen: 3.5 days
static const int64 nTargetSpacing = 60; // Newyen: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Newyen: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Newyen: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0efd59df926efa8890caf2bc1cd9b3736ba8f8a43766ea9ddb00f0a8ad294b8f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Another alt coin? Let's see how this goes";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f0d5beb7604136d702ca510d3a6fb1a9c7422c9e84fbaa6ba6c3edbb250c69e11b4250352499c2318cb802a58c6ff2ca68fd4c7ef7c3923c25c60acc4bcbc337") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519074845;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085461105;

        if (fTestNet)
        {
          block.nTime    = 1519074846;
            block.nNonce   = 386371075;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x4f8fdf9536c52580e988cda7cbb8009df6e4813bac1d4ac06ca81248380b4091"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf5, 0xc1, 0xb4, 0xd8 }; // Newyen: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NewyenMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NewyenMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NewyenMiner(CWallet *pwallet)
{
    printf("NewyenMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("newyen-miner");
    ...
    try { loop {
        ...
        printf("Running NewyenMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NewyenMiner, pwallet));
}
